...
###############################################
# Burn-cycle.sh（時間選択式・cycle別ログ・判定 完成版）
###############################################
...
case "$GPU_TIME_LABEL" in
    ...
    *) zenity --error --text="無効な GPU 実行時間です" && exit 1 ;;
esac

BIT_CFG="${BIT_CFG_DIR}/${GPU_TIME_LABEL}.cfg"

[[ ! -f "$BIT_CFG" ]] && zenity --error --text="BurnInTest cfg が存在しません\n$BIT_CFG" && exit 1
...
for ((i=1;i<=NUM_CYCLES;i++)); do
    ...
    gnome-terminal -- bash -c \
        "sudo ./bit_cmd_line_x64 -C ${BIT_CFG}" &
    BIT_PID=$!
