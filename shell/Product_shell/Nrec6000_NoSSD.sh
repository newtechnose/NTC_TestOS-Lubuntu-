...
#	mkdir -p "/home/testos/Desktop/Result/$SERIAL/burnintest"
	...
	if [ $setup_raid -eq 1 ]; then
	    while true; do
	        ...
	        drive_count=$(zenity --entry --title="Diskの本数入力" --text="ディスクの本数（2～6）を入力してください")
	        ...
	        # 入力された Disk の本数を確認します（2～6本であるかチェック）
	        if [[ "$drive_count" -ge 2 && "$drive_count" -le 6 ]]; then
	            break
	        else
	            zenity --error --title="エラー" --text="サポート外の本数です。2～6本の間で入力してください。"
	        fi
	    done
