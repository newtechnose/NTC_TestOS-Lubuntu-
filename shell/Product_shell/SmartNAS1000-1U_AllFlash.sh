...
if [ "$count" -eq "$TARGET_COUNT" ]; then
	...
	if zenity --question --title="statusファイルの初期化確認" --text="SmartNAS1000-1U(オールフラッシュモデル)の製品テストをはじめから始めますか？"; then
	    # ユーザーが「はい」を選択した場合、ファイルを初期値に戻す
	    echo "30" > "$STATUS_FILE"
...
TARGET_COUNT=30
if [ "$count" -eq "$TARGET_COUNT" ]; then
    echo "カウントが ${TARGET_COUNT} に達しました。第30群コマンドを実行します。"
...
#	mkdir -p "/home/testos/Desktop/Result/$SERIAL/burnintest"
	...
	*) raid_description="不明なRAID構成" ;;
	...
	figlet "SmartNAS1000 ALL Flash TEST START"
...
TARGET_COUNT=31
if [ "$count" -eq "$TARGET_COUNT" ]; then
    echo "カウントが ${TARGET_COUNT} に達しました。第31群コマンドを実行します。"
	...
	sudo ./bit_cmd_line_x64 -C /home/testos/BurnInTest_cfg/Allflash.cfg
	sleep 3
