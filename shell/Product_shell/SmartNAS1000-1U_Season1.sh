...
if [ "$count" -eq "$TARGET_COUNT" ]; then
	...
	10)
		...
		# さらに項目を追加する場合は、同様の処理を続ける
		printf "| %-30s | %-6s | %-50s |\n" "VD0作成" "$result" "$raid_description" >> "$txt_file"
		echo "RAID10でVD0(200GB)が構成されました。" | tee -a /home/testos/Desktop/Result/$SERIAL/Log/1_RAID_VD0.txt
		echo -e "\n\n\n\n\n"
		...
		# さらに項目を追加する場合は、同様の処理を続ける
		printf "| %-30s | %-6s | %-50s |\n" "VD1作成" "$result" "$raid_description" >> "$txt_file"
		echo "RAID10でVD1(残りの容量)が構成されました。" | tee -a /home/testos/Desktop/Result/$SERIAL/Log/2_RAID_VD1.txt
		echo -e "\n\n\n\n\n"
		sleep 1
		;;
	11)
