...
if [ "$count" -eq "$TARGET_COUNT" ]; then
	...
	if zenity --question --title="statusファイルの初期化確認" --text="SmartNAS1000-2U(オールフラッシュモデル)の製品テストをはじめから始めますか？"; then
	    # ユーザーが「はい」を選択した場合、ファイルを初期値に戻す
	    echo "33" > "$STATUS_FILE"
...
TARGET_COUNT=33
if [ "$count" -eq "$TARGET_COUNT" ]; then
    echo "カウントが ${TARGET_COUNT} に達しました。第33群コマンドを実行します。"
...
TARGET_COUNT=34
if [ "$count" -eq "$TARGET_COUNT" ]; then
    echo "カウントが ${TARGET_COUNT} に達しました。第34群コマンドを実行します。"
