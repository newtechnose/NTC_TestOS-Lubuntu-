...
if [ "$count" -eq "$TARGET_COUNT" ]; then
	...
	if zenity --question --title="statusファイルの初期化確認" --text="セコム向けAIサーバ(AMD)の製品テストをはじめから始めますか？"; then
	    # ユーザーが「はい」を選択した場合、ファイルを初期値に戻す
	    echo "21" > "$STATUS_FILE"
...
TARGET_COUNT=21
if [ "$count" -eq "$TARGET_COUNT" ]; then
    echo "カウントが ${TARGET_COUNT} に達しました。第21群コマンドを実行します。"
	...
	figlet "AI Server AMD TEST Start"
...
TARGET_COUNT=22
if [ "$count" -eq "$TARGET_COUNT" ]; then
    echo "カウントが ${TARGET_COUNT} に達しました。第22群コマンドを実行します。"
	...
	# タスク1: BurnInTest
	echo "DiskのBurnInTestを開始します。"
	sleep 5
	cd /home/testos/V4/burnintest/64bit
	sudo ./bit_cmd_line_x64 -C /home/testos/BurnInTest_cfg/AIServerAMD_Disk.cfg
	sleep 3
	echo "DiskのBurnInTestを完了しました。" 
	sleep 3
	
	sudo mv /home/testos/Desktop/Result/BiTLog2.log /home/testos/Desktop/Result/$SERIAL/Log/BurnInTest_Disk_Result.log 
	log_file="/home/testos/Desktop/Result/$SERIAL/Log/BurnInTest_Disk_Result.log"
	...
	printf "| %-30s | %-6s | %-50s |\n" "DiskのBurnInTest" "$result" "BurnInTest_Disk_Result" >> "$txt_file"
	...
	sleep 5
	cd /home/testos/V4/burnintest/64bit
	sudo ./bit_cmd_line_x64 -C /home/testos/BurnInTest_cfg/AIServerAMD.cfg
	sleep 3
	...
	while true; do
		#コマンドを実行して結果を取得
		OUTPUT=$(sudo ./nu_disp -t /dev/sda --show-info-all)
		echo "$OUTPUT"
		#PATROL SUCCESSが出力に含まれているか確認
...
if [ "$count" -eq "$TARGET_COUNT" ]; then
	...
	#完了通知
	python3 /home/testos/shell/Product_shell/secom_ai_AMD_finish.py
	# status.txtに書き込む
	echo "" > "$STATUS_FILE"
	exit 1
