...
if [ "$count" -eq "$TARGET_COUNT" ]; then
	...
	if zenity --question --title="statusファイルの初期化確認" --text="SmartNAS1000-1Uの製品テストをはじめから始めますか？"; then
	    # ユーザーが「はい」を選択した場合、ファイルを初期値に戻す
	    echo "0" > "$STATUS_FILE"
...
TARGET_COUNT=0
if [ "$count" -eq "$TARGET_COUNT" ]; then
    echo "カウントが ${TARGET_COUNT} に達しました。第0群コマンドを実行します。"
	...
	# 結果の表示
	echo "選択されたRAIDタイプID: $raid_type_id" | tee -a /home/testos/Desktop/Result/$SERIAL/raid_type.txt
...
TARGET_COUNT=1
if [ "$count" -eq "$TARGET_COUNT" ]; then
    echo "カウントが ${TARGET_COUNT} に達しました。第1群コマンドを実行します。"
	...
	1)
		# RAID type 1の設定
		sudo ./storcli64 /c0 add vd type=$raid_level size=200GB name=VD0 drives=$drive_list strip=64 awb cached ra pdcache=on | tee -a /home/testos/Desktop/Result/$SERIAL/Log/1_RAID_VD0.txt
		sleep 1
	        # Status判定
		...
		echo "RAID0でVD0(200GB)が構成されました。" | tee -a /home/testos/Desktop/Result/$SERIAL/Log/1_RAID_VD0.txt
		echo -e "\n\n\n\n\n"
		
		sudo ./storcli64 /c0 add vd type=$raid_level size=all name=VD1 drives=$drive_list strip=64 awb cached ra pdcache=on | tee -a /home/testos/Desktop/Result/$SERIAL/Log/2_RAID_VD1.txt
		sleep 1
	        # Status判定
		...
		echo "RAID0でVD1(残りの容量)が構成されました。" | tee -a /home/testos/Desktop/Result/$SERIAL/Log/2_RAID_VD1.txt
		echo -e "\n\n\n\n\n"
		sleep 1
		;;
	2)
	...
	3)
		...
		# さらに項目を追加する場合は、同様の処理を続ける
		printf "| %-15s | %-4s | %-20s |\n" "ホットスペア作成" "$result" "$raid_description" >> "$txt_file"
		echo "Slot 2,3にホットスペアが構成されました。" | tee -a /home/testos/Desktop/Result/$SERIAL/Log/3_RAID_Hotspare.txt
		echo -e "\n\n\n\n\n"
		sleep 1
		;;
	4)
		...
		# さらに項目を追加する場合は、同様の処理を続ける
		printf "| %-15s | %-4s | %-20s |\n" "VD0作成" "$result" "$raid_description" >> "$txt_file"
		echo "RAID1でVD0(200GB)が構成されました。" | tee -a /home/testos/Desktop/Result/$SERIAL/Log/1_RAID_VD0.txt
		echo -e "\n\n\n\n\n"
		...
		# さらに項目を追加する場合は、同様の処理を続ける
		printf "| %-15s | %-4s | %-20s |\n" "VD1作成" "$result" "$raid_description" >> "$txt_file"
		echo "RAID1でVD1(残りの容量)が構成されました。" | tee -a /home/testos/Desktop/Result/$SERIAL/Log/2_RAID_VD1.txt
		echo -e "\n\n\n\n\n"
		sleep 1
		;;
	5)
	...
	7)
		...
		# さらに項目を追加する場合は、同様の処理を続ける
		printf "| %-15s | %-4s | %-20s |\n" "VD0作成" "$result" "$raid_description" >> "$txt_file"
		echo "RAID5でVD0(200GB)が構成されました。" | tee -a /home/testos/Desktop/Result/$SERIAL/1_RAID_VD0.txt
		echo -e "\n\n\n\n\n"
		...
		# さらに項目を追加する場合は、同様の処理を続ける
		printf "| %-15s | %-4s | %-20s |\n" "VD1作成" "$result" "$raid_description" >> "$txt_file"
		echo "RAID5でVD1(残りの容量)が構成されました。" | tee -a /home/testos/Desktop/Result/$SERIAL/2_RAID_VD1.txt
		echo -e "\n\n\n\n\n"
		sleep 1
		;;
	8)
		...
		# さらに項目を追加する場合は、同様の処理を続ける
		printf "| %-15s | %-4s | %-20s |\n" "VD0作成" "$result" "$raid_description" >> "$txt_file"
		echo "RAID5でVD0(200GB)が構成されました。" | tee -a /home/testos/Desktop/Result/$SERIAL/Log/1_RAID_VD0.txt
		echo -e "\n\n\n\n\n"
		...
		# さらに項目を追加する場合は、同様の処理を続ける
		printf "| %-15s | %-4s | %-20s |\n" "VD1作成" "$result" "$raid_description" >> "$txt_file"
		echo "RAID5でVD1(残りの容量)が構成されました。" | tee -a /home/testos/Desktop/Result/$SERIAL/Log/2_RAID_VD1.txt
		echo -e "\n\n\n\n\n"
	...
	9)
		...
		# さらに項目を追加する場合は、同様の処理を続ける
		printf "| %-15s | %-4s | %-20s |\n" "VD0作成" "$result" "$raid_description" >> "$txt_file"
		echo "RAID6でVD0(200GB)が構成されました。" | tee -a /home/testos/Desktop/Result/$SERIAL/1_RAID_VD0.txt
		echo -e "\n\n\n\n\n"
		...
		# さらに項目を追加する場合は、同様の処理を続ける
		printf "| %-15s | %-4s | %-20s |\n" "VD1作成" "$result" "$raid_description" >> "$txt_file"
		echo "RAID6でVD1(残りの容量)が構成されました。" | tee -a /home/testos/Desktop/Result/$SERIAL/2_RAID_VD1.txt
		echo -e "\n\n\n\n\n"
		sleep 1
		;;
	10)
		...
		# さらに項目を追加する場合は、同様の処理を続ける
		printf "| %-15s | %-4s | %-20s |\n" "VD0作成" "$result" "$raid_description" >> "$txt_file"
		echo "RAID10でVD0(200GB)が構成されました。" | tee -a /home/testos/Desktop/Result/$SERIAL/1_RAID_VD0.txt
		echo -e "\n\n\n\n\n"
		...
		# さらに項目を追加する場合は、同様の処理を続ける
		printf "| %-15s | %-4s | %-20s |\n" "VD1作成" "$result" "$raid_description" >> "$txt_file"
		echo "RAID10でVD1(残りの容量)が構成されました。" | tee -a /home/testos/Desktop/Result/$SERIAL/2_RAID_VD1.txt
		echo -e "\n\n\n\n\n"
		sleep 1
		;;
	11)
		...
		# さらに項目を追加する場合は、同様の処理を続ける
		printf "| %-15s | %-4s | %-20s |\n" "VD0作成" "$result" "$raid_description" >> "$txt_file"
		echo "RAID0でVD0(200GB)が構成されました。" | tee -a /home/testos/Desktop/Result/$SERIAL/1_RAID_VD0.txt
		echo -e "\n\n\n\n\n"
		...
		# さらに項目を追加する場合は、同様の処理を続ける
		printf "| %-15s | %-4s | %-20s |\n" "VD1作成" "$result" "$raid_description" >> "$txt_file"
		echo "RAID0でVD1(残りの容量)が構成されました。" | tee -a /home/testos/Desktop/Result/$SERIAL/2_RAID_VD1.txt
		echo -e "\n\n\n\n\n"
		sleep 1
		;;
	12)
		...
		# さらに項目を追加する場合は、同様の処理を続ける
		printf "| %-15s | %-4s | %-20s |\n" "ホットスペア作成" "$result" "$raid_description" >> "$txt_file"
		echo "Slot 2にホットスペアが構成されました。" | tee -a /home/testos/Desktop/Result/$SERIAL/Log/3_RAID_Hotspare.txt
		echo -e "\n\n\n\n\n"
		sleep 1
		;;
	13)
		...
		# さらに項目を追加する場合は、同様の処理を続ける
		printf "| %-15s | %-4s | %-20s |\n" "VD0作成" "$result" "$raid_description" >> "$txt_file"
		echo "RAID1でVD0(200GB)が構成されました。" | tee -a /home/testos/Desktop/Result/$SERIAL/1_RAID_VD0.txt
		echo -e "\n\n\n\n\n"
		...
		# さらに項目を追加する場合は、同様の処理を続ける
		printf "| %-15s | %-4s | %-20s |\n" "VD1作成" "$result" "$raid_description" >> "$txt_file"
		echo "RAID1でVD1(残りの容量)が構成されました。" | tee -a /home/testos/Desktop/Result/$SERIAL/2_RAID_VD1.txt
		echo -e "\n\n\n\n\n"
		sleep 1
		;;
	14)
	...
	# 結果から速度を抽出（数値のみを取り出す）
	speed=$(echo "$result" | grep -oP '\d+(\.\d+)?(?= MB/sec)' | head -n 1)

	# 速度が500.00 MB/sec以上かどうかのチェック
	if (( $(echo "$speed >= 500.00" | bc -l) )); then
	    result="合格"
	else
	    result="不合格"
	fi
	...
	hdparm_sdc=$(sudo hdparm -ft /dev/sdb | tee -a /home/testos/Desktop/Result/$SERIAL/Log/7_hdparm_sdc.txt)
	...
	sleep 5
	cd /home/smartnas-test/V4/burnintest/
	sudo ./burnintest.sh &
	sleep 30
	...
	# Alt+Fを送信
	xdotool key --window $WINDOW_ID alt+f
	# 待機
	sleep 1
	#Enterを送信
	...
	# 待機
	sleep 5
	#smartnas_burnintest.cfgと入力
	xdotool type 'smartnas_burnintest.cfg'
	# 待機
	sleep 3
	#Enterを送信
	xdotool key --window $WINDOW_ID Return
	...
	# Alt+Tを送信
	xdotool key --window $WINDOW_ID alt+t
	# Enterを送信
	xdotool key --window $WINDOW_ID Return
	# 待機
	sleep 10
	...
	xdotool key --window $WINDOW_ID Tab
	# Resultフォルダパスを入力
	xdotool type '/home/testos/Desktop/Result/$SERIAL/Log/BurnInTest_Result.log'
	# Tabキーを送信
	xdotool key --window $WINDOW_ID Tab
	# Tabキーを送信
	xdotool key --window $WINDOW_ID Tab
	...
	# さらに項目を追加する場合は、同様の処理を続ける
	printf "| %-15s | %-4s | %-20s |\n" "BurnInTest" "$result" "cpu_maths" >> "$txt_file"
	printf "| %-15s | %-4s | %-20s |\n" "BurnInTest" " " "memory_ram" >> "$txt_file"
	printf "| %-15s | %-4s | %-20s |\n" "BurnInTest" " " "disk_sdb" >> "$txt_file"
	printf "| %-15s | %-4s | %-20s |\n" "BurnInTest" " " "disk_sdc" >> "$txt_file"
...
if [ "$count" -eq "$TARGET_COUNT" ]; then
	...
	# さらに項目を追加する場合は、同様の処理を続ける
	printf "| %-15s | %-4s | %-20s |\n" "Rebootテスト" "$result" "$COUNT_CUR" >> "$txt_file"
